...
// is schema-per-tenant, so an id from another tenant's schema is indistinguishable from one that
// was never created. Mapped to 404 by agent-os-execution-jaxrs.
public final class ExecutionNotFoundException extends RuntimeException {

  public ExecutionNotFoundException(UUID executionId) {
    super("no agent execution visible to the caller for id " + executionId);
  }
}
