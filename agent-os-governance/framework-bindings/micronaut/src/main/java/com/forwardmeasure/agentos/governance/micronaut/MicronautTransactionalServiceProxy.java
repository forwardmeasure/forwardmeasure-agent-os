...
// A direct copy of forwardmeasure-jpa-micronaut's own (package-private, so not reusable across a
// module boundary) MicronautTransactionalServiceProxy - see that class for the real precedent
// this pattern comes from (already proven in production wrapping ActorService/SystemLockService/
// AsyncTaskService there). Needed for the exact same reason: AgentServiceImpl,
// AgentAuditEventServiceImpl (via AbstractBaseServiceImpl), and AgentGovernanceServiceImpl all
// carry jakarta.transaction.Transactional but are compiled outside any Micronaut module, so
// Micronaut's compile-time AOP interceptor weaving never sees that annotation - Quarkus and Spring
// both recognise it natively on any container-managed bean regardless of construction, but
// Micronaut's interception requires bytecode generated at ITS OWN compile time, which a
// framework-neutral module never receives. This is infrastructure plumbing, not a wire contract
// model - reusing the proven shape here is not the kind of duplication the "don't hand-roll API
// contract models" rule is about.
